...
namespace HistoricalReactiveCommand
{
    ...
    public sealed class ReactiveCommandWithGroupingHistory<TParam, TResult> : ReactiveCommandBase<TParam, TResult>
    {
            ...
            IScheduler outputScheduler)
        {
            ...
            {
                var entry = CreateHistoryEntry(_param, result);
        ...
        public HistoryEntryForGroup<TParam, TResult> CreateHistoryEntry(TParam param, TResult result)
        {
            return new HistoryEntryForGroup<TParam, TResult>(
                (entry) => discardAction.Invoke(entry.Param, entry.Result).Subscribe(),
                (entry) => executeAction.Invoke(entry.Param, entry.Result).Subscribe(),
                param, result);
        }
